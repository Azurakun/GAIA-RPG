document.addEventListener('DOMContentLoaded', () => {
    ...
    async function deleteSaveFromServer(saveId) {
        await fetch(`/delete_save/${saveId}`, { method: 'DELETE' });
        renderMainMenu();
    }
    ...
    function showScreen(screen) {
        ...
        if (screen === 'start') { 
            ...
            scenarioChooser.classList.add('hidden');
        }
    }
    ...
    async function renderMainMenu() {
        ...
        if (sortedSaves.length === 0) {
            saveSlotsList.innerHTML = '<li>No saved games found.</li>';
            return;
        }
        ...
        for (const [saveId, gameState] of sortedSaves) {
            ...
            li.innerHTML = `
                <div class="save-slot-info">
                    ...
                    <span class="save-date">Saved: ${new Date(gameState.lastSaved).toLocaleString()}</span>
                </div>
                <div class="save-slot-actions">
                    <button class="menu-btn load-btn">Load</button>
                    <button class="menu-btn secondary delete-btn">Delete</button>
                </div>`;
    ...
    function loadGame(saveId, gameState) {
        ...
        processAction("Continue the story from where I left off, describing the scene.");
    }
    ...
    async function startNewGame(scenario_id, custom_text = null) {
        ...
        showScreen('game');
        displayLoadingIndicator("The threads of fate are weaving your story...");
    ...
    async function processAction(action) {
        if (!action || isRequestInProgress) return;
        
        isRequestInProgress = true;
        displayPlayerAction(action);
        actionInput.value = '';
        updateChoices([]); // Clear choices while waiting
        ...
        try {
            ...
            updateFromAIResponse(aiResponse);
        } catch (error) {
            console.error("Error processing action:", error);
            removeLoadingIndicator();
            const errorTurn = document.createElement('div');
            errorTurn.className = 'story-turn';
            errorTurn.innerHTML = `<p style="color: #ff5555;">There was an error connecting to the story server. Please try again.</p>`;
            storyLog.appendChild(errorTurn);
        } finally {
    ...
    function displayLoadingIndicator(message = "The story continues...") {
        ...
        loadingElement.innerHTML = `<p><i>${message}</i></p>`;
        storyLog.appendChild(loadingElement);
        scrollToBottom();
    }
    ...
    detailsNavButtons.forEach(btn => btn.addEventListener('click', () => openDetailsModal(btn.dataset.panel)));
    detailsModalCloseBtn.addEventListener('click', () => closeModal(detailsModal));
    
    suggestionBtn.addEventListener('click', async () => {
        if (isRequestInProgress) return;
        suggestionModalContent.innerHTML = `<p><i>Thinking of a good hint...</i></p>`;
        openModal(suggestionModal);
        const response = await fetch('/get_suggestion', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ game_state: currentGameState })
        });
        const data = await response.json();
        suggestionModalContent.innerHTML = `<p>${data.suggestion}</p>`;
    });
    ...
    function showLevelUpModal() {
        ...
        for (const stat in stats) {
            ...
            button.className = 'menu-btn level-up-btn';
    ...
    async function handleLevelUpChoice(event) {
        ...
        levelUpMessage.className = 'story-turn';
        levelUpMessage.innerHTML = `<p><strong>You feel a surge of power as your ${stat} increases!</strong></p>`;
        storyLog.appendChild(levelUpMessage);
        scrollToBottom();
    }
    ...
    saveGameBtn.addEventListener('click', async () => {
        if(currentSaveId && currentGameState) {
            ...
            const response = await fetch('/save_game', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ save_id: currentSaveId, game_state: currentGameState }) });
            saveGameBtn.textContent = response.ok ? 'Saved!' : 'Save Failed!';
            setTimeout(() => { saveGameBtn.textContent = 'Save Game'; }, 2000);
        }
    });
